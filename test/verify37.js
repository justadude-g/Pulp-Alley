...
(async () => {
  ...
  // ---- 0. The Card Type filter dropdown exists with the 15 fixed types
  // (mirroring #f-cardType — the 9 original types plus the 6 Resource/
  // Asset Card Types: Contacts, Gear, Backup, Minions, Cult, Gifts) plus
  // "All Types". ----
  const typeOptions = await page.$$eval('#gallery-type-filter option', els => els.map(e => e.value));
  assert.deepStrictEqual(
    typeOptions,
    ['', 'Leader', 'Sidekick', 'Ally', 'Follower', 'Villain', 'Creature', 'Gang', 'Custom', 'Associate', 'Contacts', 'Gear', 'Backup', 'Minions', 'Cult', 'Gifts'],
    `expected the Card Type filter to offer All Types + the 15 fixed types, got ${JSON.stringify(typeOptions)}`
  );
  ok('Card Type filter dropdown offers "All Types" plus the 15 fixed card types');
